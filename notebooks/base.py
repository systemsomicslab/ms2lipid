...
def cal_accscore_rl(df_neg_data_sel1, replacement_dict, loaded_negcolumns, loaded_negmodel):
...
def preddf_rl(df_neg_data_sel1, replacement_dict, loaded_negcolumns, loaded_negmodel):
...
def create_mspfile_fromdf(df_negx, title='EtherPC-EtherOxPC_negdata', ionmode='Negative', outputpath = None):
    if outputpath is None:
        outputpath = f"Data/msp/{title}.msp"
    else:
        outputpath = f"{outputpath}/{title}.msp"

    output_file = outputpath

    with open(output_file, 'w', newline='') as file:
        for i in range(len(df_negx)):
            df = df_negx.iloc[i]

            Metabolitename = df['Metabolitename']
            ont = df['Ontology']
            pre_mz = df['AverageMz']
            pre_type = df['Adducttype']
            RT = df['AverageRt(min)']
            formula = df['Formula']
            smiles = df['SMILES']
            inchi = df['INCHIKEY']
            ...
            file.write(f"NAME: {Metabolitename}\n")
            file.write(f"Ontology: {ont}\n")
            file.write(f"PRECURSORMZ: {pre_mz}\n")
            file.write(f"PRECURSORTYPE: {pre_type}\n")
            file.write(f"RETENTIONTIME: {RT}\n")
            file.write(f"FORMULA: {formula}\n")
            file.write(f"SMILES: {smiles}\n")
            file.write(f"INCHIKEY: {inchi}\n")
            file.write(f"IONMODE: {ionmode}\n")
            file.write(f"Num Peaks: {num}\n")
            file.write('\n'.join(ms2_split_data))
            file.write('\n')
            file.write('\n')